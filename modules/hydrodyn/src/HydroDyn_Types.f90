...
! =========  HD_ModuleMapType  =======
  TYPE, PUBLIC :: HD_ModuleMapType
    ...
    TYPE(MeshMapType)  :: S_P_2_PRP_P      !< Mesh mapping data: lumped SeaFEM loads to PRP node at (0,0,0) [-]
...
! =========  HydroDyn_MiscVarType  =======
  TYPE, PUBLIC :: HydroDyn_MiscVarType
    ...
    TYPE(SeaFEM_MiscVarType)  :: SeaFEM      !< misc var information from the Morison module [-]
...
! =========  HydroDyn_InputType  =======
  TYPE, PUBLIC :: HydroDyn_InputType
    ...
    TYPE(MeshType)  :: Mesh      !< Motions at the Platform reference point in the inertial frame [-]
...
! =========  HydroDyn_OutputType  =======
  TYPE, PUBLIC :: HydroDyn_OutputType
    ...
    TYPE(MeshType)  :: PRPMesh      !< Point Loads at the WAMIT reference point(s) in the inertial frame [-]
    TYPE(MeshType)  :: Mesh      !< Point Loads at the WAMIT reference point(s) in the inertial frame [-]
...
IF (ALLOCATED(SrcInitInputData%AddF0)) THEN
  ...
  i2_l = LBOUND(SrcInitInputData%AddF0,2)
  i2_u = UBOUND(SrcInitInputData%AddF0,2)
  IF (.NOT. ALLOCATED(DstInitInputData%AddF0)) THEN 
    ALLOCATE(DstInitInputData%AddF0(i1_l:i1_u,i2_l:i2_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstInitInputData%AddF0.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstInitInputData%AddF0 = SrcInitInputData%AddF0
ENDIF
IF (ALLOCATED(SrcInitInputData%AddCLin)) THEN
  i1_l = LBOUND(SrcInitInputData%AddCLin,1)
  i1_u = UBOUND(SrcInitInputData%AddCLin,1)
  i2_l = LBOUND(SrcInitInputData%AddCLin,2)
  i2_u = UBOUND(SrcInitInputData%AddCLin,2)
  i3_l = LBOUND(SrcInitInputData%AddCLin,3)
  i3_u = UBOUND(SrcInitInputData%AddCLin,3)
  IF (.NOT. ALLOCATED(DstInitInputData%AddCLin)) THEN 
    ALLOCATE(DstInitInputData%AddCLin(i1_l:i1_u,i2_l:i2_u,i3_l:i3_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstInitInputData%AddCLin.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstInitInputData%AddCLin = SrcInitInputData%AddCLin
ENDIF
IF (ALLOCATED(SrcInitInputData%AddBLin)) THEN
  i1_l = LBOUND(SrcInitInputData%AddBLin,1)
  i1_u = UBOUND(SrcInitInputData%AddBLin,1)
  i2_l = LBOUND(SrcInitInputData%AddBLin,2)
  i2_u = UBOUND(SrcInitInputData%AddBLin,2)
  i3_l = LBOUND(SrcInitInputData%AddBLin,3)
  i3_u = UBOUND(SrcInitInputData%AddBLin,3)
  IF (.NOT. ALLOCATED(DstInitInputData%AddBLin)) THEN 
    ALLOCATE(DstInitInputData%AddBLin(i1_l:i1_u,i2_l:i2_u,i3_l:i3_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstInitInputData%AddBLin.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstInitInputData%AddBLin = SrcInitInputData%AddBLin
ENDIF
IF (ALLOCATED(SrcInitInputData%AddBQuad)) THEN
  i1_l = LBOUND(SrcInitInputData%AddBQuad,1)
  i1_u = UBOUND(SrcInitInputData%AddBQuad,1)
  i2_l = LBOUND(SrcInitInputData%AddBQuad,2)
  i2_u = UBOUND(SrcInitInputData%AddBQuad,2)
  i3_l = LBOUND(SrcInitInputData%AddBQuad,3)
  i3_u = UBOUND(SrcInitInputData%AddBQuad,3)
  IF (.NOT. ALLOCATED(DstInitInputData%AddBQuad)) THEN 
    ALLOCATE(DstInitInputData%AddBQuad(i1_l:i1_u,i2_l:i2_u,i3_l:i3_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstInitInputData%AddBQuad.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
...
ENDIF
      ...
      CALL Current_CopyInitInput( SrcInitInputData%Current, DstInitInputData%Current, CtrlCode, ErrStat2, ErrMsg2 )
         CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg,RoutineName)
         IF (ErrStat>=AbortErrLev) RETURN
IF (ALLOCATED(SrcInitInputData%PotFile)) THEN
  i1_l = LBOUND(SrcInitInputData%PotFile,1)
  i1_u = UBOUND(SrcInitInputData%PotFile,1)
  IF (.NOT. ALLOCATED(DstInitInputData%PotFile)) THEN 
    ALLOCATE(DstInitInputData%PotFile(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstInitInputData%PotFile.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstInitInputData%PotFile = SrcInitInputData%PotFile
ENDIF
    DstInitInputData%nWAMITObj = SrcInitInputData%nWAMITObj
    DstInitInputData%vecMultiplier = SrcInitInputData%vecMultiplier
...
IF (ALLOCATED(SrcInitInputData%WAMITULEN)) THEN
  ...
  IF (.NOT. ALLOCATED(DstInitInputData%WAMITULEN)) THEN 
    ALLOCATE(DstInitInputData%WAMITULEN(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstInitInputData%WAMITULEN.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
...
ENDIF
IF (ALLOCATED(InitInputData%AddF0)) THEN
  DEALLOCATE(InitInputData%AddF0)
ENDIF
IF (ALLOCATED(InitInputData%AddCLin)) THEN
  DEALLOCATE(InitInputData%AddCLin)
ENDIF
IF (ALLOCATED(InitInputData%AddBLin)) THEN
  DEALLOCATE(InitInputData%AddBLin)
ENDIF
IF (ALLOCATED(InitInputData%AddBQuad)) THEN
  DEALLOCATE(InitInputData%AddBQuad)
ENDIF
...
IF (ALLOCATED(InitInputData%PotFile)) THEN
  DEALLOCATE(InitInputData%PotFile)
ENDIF
IF (ALLOCATED(InitInputData%PtfmVol0)) THEN
  DEALLOCATE(InitInputData%PtfmVol0)
ENDIF
IF (ALLOCATED(InitInputData%WAMITULEN)) THEN
  DEALLOCATE(InitInputData%WAMITULEN)
ENDIF
...
ENDIF
      CALL MeshCopy( SrcInitOutputData%Mesh, DstInitOutputData%Mesh, CtrlCode, ErrStat2, ErrMsg2 )
         CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
         IF (ErrStat>=AbortErrLev) RETURN
      CALL MeshCopy( SrcInitOutputData%PRPMesh, DstInitOutputData%PRPMesh, CtrlCode, ErrStat2, ErrMsg2 )
         CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
         IF (ErrStat>=AbortErrLev) RETURN
...
ENDIF
  ...
  CALL MeshDestroy( InitOutputData%PRPMesh, ErrStat, ErrMsg )
...
! 
   ...
   ErrMsg  = ""
      ...
      CALL NWTC_Library_Copymeshmaptype( SrcHD_ModuleMapTypeData%M_P_2_PRP_P, DstHD_ModuleMapTypeData%M_P_2_PRP_P, CtrlCode, ErrStat2, ErrMsg2 )
         CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg,RoutineName)
         IF (ErrStat>=AbortErrLev) RETURN
      CALL NWTC_Library_Copymeshmaptype( SrcHD_ModuleMapTypeData%S_P_2_PRP_P, DstHD_ModuleMapTypeData%S_P_2_PRP_P, CtrlCode, ErrStat2, ErrMsg2 )
         CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg,RoutineName)
         IF (ErrStat>=AbortErrLev) RETURN
...
! 
  ...
  CALL NWTC_Library_Destroymeshmaptype( HD_ModuleMapTypeData%S_P_2_PRP_P, ErrStat, ErrMsg )
 ...
 ! buffers to store subtypes, if any
  ...
  Int_BufSz  = 0
   ! Allocate buffers for subtypes, if any (we'll get sizes from these) 
      ...
      IF(ALLOCATED(Int_Buf)) THEN ! M_P_2_PRP_P
         ...
         DEALLOCATE(Int_Buf)
      END IF
      Int_BufSz   = Int_BufSz + 3  ! S_P_2_PRP_P: size of buffers for each call to pack subtype
      CALL NWTC_Library_Packmeshmaptype( Re_Buf, Db_Buf, Int_Buf, InData%S_P_2_PRP_P, ErrStat2, ErrMsg2, .TRUE. ) ! S_P_2_PRP_P 
        CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
        IF (ErrStat >= AbortErrLev) RETURN

      IF(ALLOCATED(Re_Buf)) THEN ! S_P_2_PRP_P
         Re_BufSz  = Re_BufSz  + SIZE( Re_Buf  )
         DEALLOCATE(Re_Buf)
      END IF
      IF(ALLOCATED(Db_Buf)) THEN ! S_P_2_PRP_P
         Db_BufSz  = Db_BufSz  + SIZE( Db_Buf  )
         DEALLOCATE(Db_Buf)
      END IF
      IF(ALLOCATED(Int_Buf)) THEN ! S_P_2_PRP_P
         Int_BufSz = Int_BufSz + SIZE( Int_Buf )
         DEALLOCATE(Int_Buf)
      END IF
  IF ( Re_BufSz  .GT. 0 ) THEN 
  ...
  Int_Xferred = 1
      ...
      ELSE
        IntKiBuf( Int_Xferred ) = 0; Int_Xferred = Int_Xferred + 1
      ENDIF
      CALL NWTC_Library_Packmeshmaptype( Re_Buf, Db_Buf, Int_Buf, InData%S_P_2_PRP_P, ErrStat2, ErrMsg2, OnlySize ) ! S_P_2_PRP_P 
        CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
        IF (ErrStat >= AbortErrLev) RETURN
 ...
 ! buffers to store meshes, if any
  ...
  Int_Xferred  = 1
      ...
      IF(Buf_size > 0) THEN
        ...
        Int_Xferred = Int_Xferred + Buf_size
      END IF
      CALL NWTC_Library_Unpackmeshmaptype( Re_Buf, Db_Buf, Int_Buf, OutData%S_P_2_PRP_P, ErrStat2, ErrMsg2 ) ! S_P_2_PRP_P 
        CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
        IF (ErrStat >= AbortErrLev) RETURN
...
ENDIF
      ...
      CALL SeaFEM_CopyOtherState( SrcOtherStateData%SeaFEM, DstOtherStateData%SeaFEM, CtrlCode, ErrStat2, ErrMsg2 )
         CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg,RoutineName)
         IF (ErrStat>=AbortErrLev) RETURN
      CALL MeshCopy( SrcOtherStateData%MrsnMesh_position, DstOtherStateData%MrsnMesh_position, CtrlCode, ErrStat2, ErrMsg2 )
         CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
         IF (ErrStat>=AbortErrLev) RETURN
    ...
    DstOtherStateData%F_Hydro = SrcOtherStateData%F_Hydro
      CALL MeshCopy( SrcOtherStateData%AllHdroOrigin, DstOtherStateData%AllHdroOrigin, CtrlCode, ErrStat2, ErrMsg2 )
         CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
         IF (ErrStat>=AbortErrLev) RETURN
...
ENDIF
  ...
  CALL MeshDestroy( OtherStateData%MrsnMesh_position, ErrStat, ErrMsg )
  CALL HydroDyn_Destroyhd_modulemaptype( OtherStateData%HD_MeshMap, ErrStat, ErrMsg )
  CALL MeshDestroy( OtherStateData%AllHdroOrigin, ErrStat, ErrMsg )
...
ENDIF
      ...
      CALL Morison_CopyMisc( SrcMiscData%Morison, DstMiscData%Morison, CtrlCode, ErrStat2, ErrMsg2 )
         CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg,RoutineName)
         IF (ErrStat>=AbortErrLev) RETURN
      CALL SeaFEM_CopyMisc( SrcMiscData%SeaFEM, DstMiscData%SeaFEM, CtrlCode, ErrStat2, ErrMsg2 )
         CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg,RoutineName)
         IF (ErrStat>=AbortErrLev) RETURN
...
ENDIF
  ...
  CALL SeaFEM_DestroyMisc( MiscData%SeaFEM, ErrStat, ErrMsg )
...
ENDIF
 ...
 ! buffers to store subtypes, if any
  ...
  END IF
      ...
      END IF
      Int_BufSz   = Int_BufSz + 3  ! SeaFEM: size of buffers for each call to pack subtype
      CALL SeaFEM_PackMisc( Re_Buf, Db_Buf, Int_Buf, InData%SeaFEM, ErrStat2, ErrMsg2, .TRUE. ) ! SeaFEM 
        CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
        IF (ErrStat >= AbortErrLev) RETURN
  ...
  END IF
      ...
      ELSE
        IntKiBuf( Int_Xferred ) = 0; Int_Xferred = Int_Xferred + 1
      ENDIF
      CALL SeaFEM_PackMisc( Re_Buf, Db_Buf, Int_Buf, InData%SeaFEM, ErrStat2, ErrMsg2, OnlySize ) ! SeaFEM 
        CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
        IF (ErrStat >= AbortErrLev) RETURN
 ...
 ! buffers to store meshes, if any
  ...
  END IF
      ...
      IF(Buf_size > 0) THEN
        ...
        Int_Xferred = Int_Xferred + Buf_size
      END IF
      CALL SeaFEM_UnpackMisc( Re_Buf, Db_Buf, Int_Buf, OutData%SeaFEM, ErrStat2, ErrMsg2 ) ! SeaFEM 
        CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
        IF (ErrStat >= AbortErrLev) RETURN
